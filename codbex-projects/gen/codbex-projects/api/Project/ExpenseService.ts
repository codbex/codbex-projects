...
import { ExpenseRepository, ExpenseEntityOptions } from "../../dao/Project/ExpenseRepository";
import { user } from "sdk/security"
import { ForbiddenError } from "../utils/ForbiddenError";
import { ValidationError } from "../utils/ValidationError";
import { HttpUtils } from "../utils/HttpUtils";

const validationModules = await Extensions.loadExtensionModules("codbex-projects-Project-Expense", ["validate"]);
...
class ExpenseService {
    ...
    public getAll(_: any, ctx: any) {
        try {
            ...
            const options: ExpenseEntityOptions = {
    ...
    public create(entity: any) {
        try {
            ...
            response.setHeader("Content-Location", "/services/ts/codbex-projects/gen/codbex-projects/api/Project/ExpenseService.ts/" + entity.Id);
    ...
    public getById(_: any, ctx: any) {
        try {
            ...
            if (entity) {
                ...
                HttpUtils.sendResponseNotFound("Expense not found");
            }
        } catch (error: any) {
            this.handleError(error);
        }
    ...
    private checkPermissions(operationType: string) {
        if (operationType === "read" && !(user.isInRole("codbex-projects.Project.ExpenseReadOnly") || user.isInRole("codbex-projects.Project.ExpenseFullAccess"))) {
            throw new ForbiddenError();
        }
        if (operationType === "write" && !user.isInRole("codbex-projects.Project.ExpenseFullAccess")) {
            throw new ForbiddenError();
        }
    }
