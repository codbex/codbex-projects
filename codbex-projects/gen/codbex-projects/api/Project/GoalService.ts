...
class GoalService {
    ...
    public getAll(_: any, ctx: any) {
        try {
            ...
            const options: GoalEntityOptions = {
    ...
    public getById(_: any, ctx: any) {
        try {
            ...
            if (entity) {
                ...
                HttpUtils.sendResponseNotFound("Goal not found");
            }
        } catch (error: any) {
            this.handleError(error);
        }
    ...
    private checkPermissions(operationType: string) {
        if (operationType === "read" && !(user.isInRole("codbex-projects.Project.GoalReadOnly") || user.isInRole("codbex-projects.Project.GoalFullAccess"))) {
            throw new ForbiddenError();
        }
        if (operationType === "write" && !user.isInRole("codbex-projects.Project.GoalFullAccess")) {
            throw new ForbiddenError();
        }
    }
    ...
    private validateEntity(entity: any): void {
        ...
        if (entity.Importance?.length > 20) {
            throw new ValidationError(`The 'Importance' exceeds the maximum length of [20] characters`);
        }
