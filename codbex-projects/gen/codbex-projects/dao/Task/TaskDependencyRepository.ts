...
export class TaskDependencyRepository {
    ...
    private async triggerEvent(data: TaskDependencyEntityEvent | TaskDependencyUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("codbex-projects-Task-TaskDependency", ["trigger"]);
        ...
        producer.topic("codbex-projects-Task-TaskDependency").send(JSON.stringify(data));
    }
}
