	...
	.controller('PageController', ($scope, $http, ViewParameters, LocaleService, EntityService) => {
		...
		$scope.create = () => {
			...
			EntityService.create(entity).then((response) => {
				...
				$scope.$evalAsync(() => {
					$scope.errorMessage = LocaleService.t('codbex-projects:codbex-projects-model.messages.error.unableToCreate', { name: '$t(codbex-projects:codbex-projects-model.t.DELIVERABLE)', message: message });
				});
		...
		$scope.update = () => {
			...
			EntityService.update(id, entity).then((response) => {
				...
				const message = error.data ? error.data.message : '';
				$scope.$evalAsync(() => {
					$scope.errorMessage = LocaleService.t('codbex-projects:codbex-projects-model.messages.error.unableToUpdate', { name: '$t(codbex-projects:codbex-projects-model.t.DELIVERABLE)', message: message });
				});
				console.error('EntityService:', error);
			});
		};
		...
		$scope.cancel = () => {
			$scope.entity = {};
			$scope.action = 'select';
			Dialogs.closeWindow({ id: 'Deliverable-details' });
		};
