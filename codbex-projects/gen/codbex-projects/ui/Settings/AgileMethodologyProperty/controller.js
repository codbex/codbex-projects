angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', '$http', 'messageHub', 'entityApi', 'Extensions', function ($scope, $http, messageHub, entityApi, Extensions) {
		...
		Extensions.get('dialogWindow', 'codbex-projects-custom-action').then(function (response) {
			$scope.pageActions = response.filter(e => e.perspective === "Settings" && e.view === "AgileMethodologyProperty" && (e.type === "page" || e.type === undefined));
			$scope.entityActions = response.filter(e => e.perspective === "Settings" && e.view === "AgileMethodologyProperty" && e.type === "entity");
		});
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				if (response.status != 200) {
					messageHub.showAlertError("AgileMethodologyProperty", `Unable to count AgileMethodologyProperty: '${response.message}'`);
					return;
				}
				if (response.data) {
				...
				request.then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("AgileMethodologyProperty", `Unable to list/filter AgileMethodologyProperty: '${response.message}'`);
						return;
					}
		...
		$scope.deleteEntity = function (entity) {
			...
			messageHub.showDialogAsync(
				'Delete AgileMethodologyProperty?',
				`Are you sure you want to delete AgileMethodologyProperty? This action cannot be undone.`,
				[{
					id: "delete-btn-yes",
					type: "emphasized",
					label: "Yes",
				...
				if (msg.data === "delete-btn-yes") {
					entityApi.delete(id).then(function (response) {
						if (response.status != 204) {
							messageHub.showAlertError("AgileMethodologyProperty", `Unable to delete AgileMethodologyProperty: '${response.message}'`);
							return;
						}
						$scope.loadPage($scope.dataPage, $scope.filter);
